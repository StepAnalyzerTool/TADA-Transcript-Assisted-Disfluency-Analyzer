...
def render_summary_consolidator() -> None:
    ...
    with st.expander("How consolidation works", expanded=True):
        st.markdown(
            "1. Upload at least two Excel workbooks exported from TADA.\n"
            "2. TADA reads only the `Session_Summary` worksheet from each workbook. It does not combine occurrence-level coding records or transcripts.\n"
...
def render_ioa_calculator() -> None:
    ...
    with st.expander("Requirements for an IOA comparison", expanded=True):
        st.markdown(
            "1. Both reviewers must code the transcript independently and download their own TADA Excel record.\n"
            "2. One record must be exported with reviewer role **Primary** and the other with reviewer role **Secondary**.\n"
            "3. The participant ID, session information, and transcript hash must match across the two files. "
    ...
    with st.expander("How agreement is calculated"):
        st.markdown(
            "- **Decision agreement:** The numerator is the number of matched candidate occurrences for which both reviewers selected the same status: accepted by both or rejected by both. "
            "The denominator is the total number of unique matched and unmatched candidate occurrences across the two records. An occurrence found in only one record is counted in the denominator but not the numerator.\n"
            "- **Occurrence agreement:** The numerator is the number of occurrences accepted by both reviewers. The denominator is the number accepted by either or both reviewers.\n"
            "- **Total-count agreement:** The smaller total number of accepted occurrences is divided by the larger total number of accepted occurrences.\n\n"
            "Each quotient is multiplied by 100 and reported as a percentage."
        )
...
with st.sidebar:
    ...
    with st.expander("How target categories affect counting"):
        st.markdown(
            "TADA searches for literal, case-insensitive occurrences of every configured target. "
            "Assign recognizable words or phrases to **Lexical** and vocalizations that should not count as words to **Nonlexical**.\n\n"
            "Lexical targets remain in the total spoken-word denominator whether the reviewer accepts or rejects them as disfluencies. "
            "Configured nonlexical targets are excluded from the total spoken-word denominator whether accepted or rejected. "
            "Changing a target's category therefore changes the denominator used for rates per 100 words."
        )
...
with st.expander("What speaker selection and text deletion change"):
    st.markdown(
        "When reliable repeated speaker labels are detected, selecting one speaker limits the working transcript to text assigned to that label. "
        "Speaker labels generated by transcription software may be incorrect, so verify the selected text.\n\n"
        "Delete speech produced by other speakers and any material outside the intended observation from the editable working copy. "
        "Deleted text is excluded from the word count, target search, disfluency counts, and all calculated rates. "
        "Deleting text does not modify the uploaded file. The exported workbook retains both the initially selected text and the final analyzed text."
    )
...
st.info(
    "Every automatically detected candidate is accepted by default. Review every highlighted occurrence. "
    "Click a candidate to reject it; click it again to return it to accepted."
)
st.caption("Blue = accepted lexical · Gold = accepted nonlexical · Gray = rejected")
with st.expander("What accepting and rejecting change"):
    st.markdown(
        "An **accepted** candidate is included in the disfluency count and in both disfluency-rate numerators. "
        "A **rejected** candidate is excluded from disfluency counts and rate numerators but remains documented in the exported `Occurrence_Review` worksheet.\n\n"
        "Rejecting a lexical candidate does not remove that word or phrase from the transcript or total spoken-word denominator. "
        "A configured nonlexical target remains excluded from the spoken-word denominator whether accepted or rejected. "
        "To remove material from the transcript and denominator entirely, delete it from the editable working copy above rather than rejecting it here."
    )
component_findings = [
    {
        "id": candidate_id,
...
st.caption(
    "The table records the current accepted or rejected status of each detected candidate. "
    "Reviewer notes do not change any calculation."
)
...
with st.expander("How manual additions are counted"):
    st.markdown(
        "Use this form only for a disfluency that should be counted but is not present among the automatically detected candidates. "
        "A manually added occurrence is accepted immediately and is included in disfluency counts and rates.\n\n"
        "Select **Lexical** or **Nonlexical** to determine which category total includes the manually added occurrence. "
        "A manual addition does not insert text into the analyzed transcript and does not change the total spoken-word denominator. "
        "If an occurrence was added in error, select **Remove** in the manual-occurrence table and then select **Remove selected occurrence(s)** before export."
    )
...
st.header("4. Results")
with st.expander("Counting rules and formulas"):
    st.markdown(
        "**Total spoken words** is the number of word tokens in the final analyzed text after configured nonlexical targets, web addresses, and recognized non-speech annotations are excluded. "
        "Accepted and rejected lexical targets remain in this word count.\n\n"
        "**Accepted disfluencies** is the number of accepted automatically detected candidates plus accepted manually added occurrences. Rejected candidates are not included.\n\n"
        "**Disfluencies per 100 words** = (accepted disfluencies ÷ total spoken words) × 100.\n\n"
        "**Disfluencies per minute** = accepted disfluencies ÷ verified duration in minutes.\n\n"
        "The same total spoken-word and duration denominators are used for the overall, lexical, nonlexical, and individual-target rates. "
        "A configured target with no accepted occurrences is reported as zero. A rate that cannot be calculated because its denominator is unavailable is displayed as **Not available** or an em dash."
    )
m1, m2, m3, m4, m5 = st.columns(5)
m1.metric("Total spoken words", f"{len(words):,}")
...
st.header("5. Export")
with st.expander("Information stored in the Excel workbook"):
    st.markdown(
